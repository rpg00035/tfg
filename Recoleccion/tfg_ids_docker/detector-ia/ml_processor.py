...
REDIS_HOST = os.getenv("ML_REDIS_HOST", "34.175.144.41") # ¡USA LA IP ESTÁTICA!
REDIS_PORT = int(os.getenv("ML_REDIS_PORT", 6379))
REDIS_QUEUE_NAME = os.getenv("ML_REDIS_QUEUE", "argus_flow_data")

COLUMN_NAMES_STR = "stime,proto,sport,dport,state,ltime,spkts,dpkts,sbytes,dbytes,sttl,dttl,sload,dload,sloss,dloss,sintpkt,dintpkt,sjit,djit,stcpb,dtcpb,tcprtt,synack,ackdat,smeansz,dmeansz,dur"
...
def procesar_datos_flujo(csv_line):
    try:
        ...
        if model is not None and datos_para_predecir is not None:
            ...
            # Asumiendo que 0 es Normal y 1 es Attack (¡verifica esto con tu attack_type_map.json!)
            if predicted_index == 1: # Si predice Ataque
                print(f"ALERTA DETECTADA (Índice: {predicted_index}):", file=sys.stderr)
                print(f"  Línea CSV original: {csv_line}", file=sys.stderr)
                print(f"  Datos preprocesados para el modelo (shape {datos_para_predecir.shape}):\n{datos_para_predecir}", file=sys.stderr)
            else:
                print(f"Predicción Normal (Índice: {predicted_index}).", file=sys.stderr)
